...
class MarketingEntity(BaseModel):
    ...
    control: str
    product: str
    category: Optional[MarketingCategoryEnum] = Field(default=None)
    year: int
    production: int
    ...
    def set_category(cls, values):
        ...
        if not values.category and values.control:
            try:
                category_control = MarketingEntity.extract_control(values.control).upper()
                values.category = MarketingCategoryEnum[category_control]
            except KeyError:
